...
foo <- function(x) {
    ...
    phylogeny$bds_likelihood(lambda_hat, mu_hat, x[1], sampling_fraction, sd, num_speciation_classes, num_extinction_classes, tol, FALSE, condition_survival, condition_marginal_survival, condition_root_speciation, extinction_approximation)
}
...
p <- ggplot(dfx, aes(x = eta, y = logl, linetype = shifts)) +
    ...
    ggtitle("5 sp and 1 mu (RPesto)")


ggsave("figures/likelhood_curve_conifers2.pdf", p, width = 120, height = 90, units = "mm")
